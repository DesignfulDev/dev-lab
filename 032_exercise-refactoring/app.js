...
let fillTable = obj => {
	let tableHead = document.querySelectorAll('#table th'),
		tableBody = document.querySelectorAll('#table td'),
		methodColumn = [],
		descriptionColumn = [];

	// Fill headers

	tableHead[0].innerHTML = obj.methodHead;
	tableHead[1].innerHTML = obj.descHead;

	// Select columns

	for (let i = 0; i < tableBody.length; i++) {
		i % 2 === 0
			? methodColumn.push(tableBody[i])
			: descriptionColumn.push(tableBody[i]);
	}

	// Fill comlumns

	for (let j = 0; j < methodColumn.length; j++) {
		methodColumn[j].innerHTML = obj.methods[j];
		descriptionColumn[j].innerHTML = obj.descriptions[j];
	}
};
